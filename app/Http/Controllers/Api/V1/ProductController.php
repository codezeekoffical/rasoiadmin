...
class ProductController extends Controller
{
    ...
    public function get_popular_products(Request $request)
    {
     ...
     public function get_date_wise_products(Request $request)
    {
        if (!$request->hasHeader('zoneId')) {
            $errors = [];
            array_push($errors, ['code' => 'zoneId', 'message' => trans('messages.zone_id_required')]);
        ...
        $zone_id= $request->header('zoneId');
        $products = ProductLogic::datewise_products($zone_id, $request['limit'], $request['offset'], $type,$request['date']);
        $products['products'] = Helpers::product_data_formatting($products['products'], true, false, app()->getLocale());
        return response()->json($products, 200);
    }
    ...
    public function get_upcoming_products(Request $request)
    {
        ...
        $zone_id= $request->header('zoneId');
        $products = ProductLogic::upcoming_products($zone_id, $request['limit'], $request['offset'], 'all');
        $products['products'] = Helpers::product_data_formatting($products['products'], true, false, app()->getLocale());
        return response()->json($products, 200);
    } 
    public function get_today_products(Request $request)
     {
        if (!$request->hasHeader('zoneId')) {
            $errors = [];
            array_push($errors, ['code' => 'zoneId', 'message' => trans('messages.zone_id_required')]);
        ...
        $zone_id= $request->header('zoneId');
        $products = ProductLogic::today_products($zone_id, $request['limit'], $request['offset'], 'all');
        $products['products'] = Helpers::product_data_formatting($products['products'], true, false, app()->getLocale());
        return response()->json($products, 200);
    }  
     ...
     public function get_special_products(Request $request)
    {
        ...
        $zone_id= $request->header('zoneId');
        $products = ProductLogic::special_products($zone_id, $request['limit'], $request['offset'], 'all');
        $products['products'] = Helpers::product_data_formatting($products['products'], true, false, app()->getLocale());
        return response()->json($products, 200);
    }    
