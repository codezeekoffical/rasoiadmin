     ...
     //   }
      ...
      //  }
        ...
        foreach ($request['cart'] as $c) {
            ...
            if ($c['item_campaign_id'] != null) {
                ...
                if ($product) {
                    ...
                    if(!$orderdatetaken){
                     if($product['today'] == 1){
                                            $available_time = $product['available_time_starts'];
                                          //  $available_date = $product['available_date'];


                                            $start = strtotime('00:00:00');
                                            $end = strtotime($available_time);
                                            $minutes = ($end - $start) / 60;
                                           $date = strtotime(now());
                                           $date = date('Y-m-d',$date);
                                           $new_date = date('Y-m-d H:i:s',(strtotime ( '+'.$minutes.'minutes' , strtotime ( $date) ) ));
                                           $available_date = $new_date;
                                        //    $now = new DateTime();
                                          //   $available_date = date("d-m-y h:i:s");
                                         //   $available_date = date('y-m-d H:i:s', strtotime($available_date));

                    }else{
                        if($product['upcoming'] == 1){
                            if($product['available_date'] == null){
                                 $time = strtotime($product['delivery_date']);    
        ...
        try {
            ...
            $order->available_date = $available_date;
