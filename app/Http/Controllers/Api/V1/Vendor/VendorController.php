...
class VendorController extends Controller
{
    ...
    public function get_current_orders(Request $request)
    {
        ...
        ->where(function($query)use($vendor){
            ...
            {
                $query->whereIn('order_status', ['pending','confirmed', 'processing', 'handover','picked_up'])
    ...
    public function get_products(Request $request,$status)
    {
        ...
        $paginator = Food::withoutGlobalScope('translate')->type($type)->where($status,1)->where('restaurant_id', $request['vendor']->restaurants[0]->id)->latest()->paginate($limit, ['*'], 'page', $offset);
        $data = [
            'total_size' => $paginator->total(),
            'limit' => $limit,
            'offset' => $offset,
