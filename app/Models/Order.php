...
class Order extends Model
{
    ...
    public function scopeOrderDeliveryTime($query, $interval)
    {
        return $query->where(function($query)use($interval){
            $query->whereRaw('created_at <> delivery_date')->where(function($q) use ($interval) {
            $q->whereBetween('available_date', [Carbon::now()->toDateTimeString(),Carbon::now()->addMinutes($interval)->toDateTimeString()]); 
            })->orWhere('available_date','<',Carbon::now()->toDateTimeString());
        })->orWhereRaw('created_at = available_date');
