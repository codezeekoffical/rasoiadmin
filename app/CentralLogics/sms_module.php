...
class SMS_module
{
    public static function send($receiver, $otp)
    {
        ...
        $config = self::get_settings('custom_sms');
        if (isset($config) && $config['status'] == 1) {
            $response = self::custom_sms($receiver, $otp);
            return $response;
        }
   ...
   public static function custom_sms($receiver,$otp){
       $config = self::get_settings('custom_sms');
        ...
        if (isset($config) && $config['status'] == 1) {
           // $receiver = str_replace($receiver);
            ...
            $country_code = '+91';
             ...
             $postData = ['mobile' => preg_replace('/^\+?91|\|1|\D/', '', ($receiver))];
            ...
            DB::table('business_settings')->updateOrInsert(['key' => 'custom_sms'], [
                'key' => 'custom_sms',
                'value' => json_encode([
                    'status' => 0,
                ]),
